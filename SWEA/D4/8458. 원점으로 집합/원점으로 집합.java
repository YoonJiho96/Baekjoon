...
1. 2차원 배열을 1차원 배열로 만들 순 없을까?
	=> 아예 원점으로부터의 맨하탄 거리를 계산해서 0 ~ >> 양의 정수로 표현
2. 모두가 원점으로 올 수 없는 경우는?
	=> 모든 점의 거리가 모두 짝수이거나 모두 홀수여야 한다.
3. 정말 모든 점이 한꺼번에 다 움직이면서 따져야 할까?
...
public class Solution {
	static int T, N, cnt;
	static int[] point; // 1차원 배열(원점으로부터 맨하탄 거리 저장)
	...
	public static void main(String[] args) throws Exception {
		...
		for (int t = 1; t <= T; t++) {
			...
			int maxLen = point[0]; // 첫 번재 점을 최초 지정

			boolean stop = false; // 입력으 ㄹ따져보고 진행할 필요 X 이면 true
			...
			int sum = 0; // 누적 거리
			while (true) {
				if (sum == maxLen || (sum > maxLen && (sum - maxLen) % 2 == 0))
					break;
				// 원점 도달 break
				sum += ++cnt;
			}
